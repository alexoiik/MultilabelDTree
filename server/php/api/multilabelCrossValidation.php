<?php
    ...
    if(($open_file = fopen($file_path, "r")) !== FALSE) {
        ...
        // Final Validation > Features Existance.
        for($i = 0; $i < count($features); $i++) {
            ...
            if($found == 0) {
                header("HTTP/1.1 400 Bad Request");
                print json_encode(['errormesg'=>"Feature $features[$i] doesn't exist."]);
                exit;
            }
        }

        // Final Validation > Labels Existance.
        for($x = 0; $x < count($labels); $x++) {
            $found = 0;
            for($y = 0; $y < count($fields); $y++) {
                if($labels[$x] == $fields[$y]) {
                    $found++;
                }
            }
            if($found == 0) {
                header("HTTP/1.1 400 Bad Request");
                print json_encode(['errormesg'=>"Label $labels[$x] doesn't exist."]);
                exit;
            }
        }
